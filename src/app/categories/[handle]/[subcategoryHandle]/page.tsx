...
export default function SubcategoryPage({
  ...
  params: { handle: string; subcategoryHandle: string };
}) {
  ...
  useEffect(() => {
    const fetchCategoryAndProducts = async () => {
      try {
        ...
        // Fetch categories to find the category details
        const categoriesResponse = await fetch('/api/categories');
        const categoriesData = await categoriesResponse.json();

        // Find parent category from hierarchy (has 'children') and flat list
        ...
        const parentCategory = hierarchicalCategories.find(
        ...
        // Find subcategory from hierarchy children (case-insensitive)
        const subcategory = parentCategory.children?.find(
          (c: any) => c.handle.toLowerCase() === params.subcategoryHandle.toLowerCase()
        );

        if (!subcategory) {
          // Fallback: search flat list by handle + parent_category_id
          const fallbackSub = allCategories.find(
            (c: any) =>
              c.handle.toLowerCase() === params.subcategoryHandle.toLowerCase() &&
          ...
          setSubcategoryName(fallbackSub.name);
          setSubcategoryDescription(fallbackSub.description || '');

          const productsResponse = await medusaClient.products.list({
            category_id: [fallbackSub.id],
        ...
        if (fetchedProducts.length === 0) {
          ...
          fetchedProducts = allProductsResponse.products?.filter((p: any) => {
            ...
            return collectionTitle.includes(subcatName) || subcatName.includes(collectionTitle);
          }) || [];
        }
  ...
  }, [params.handle, params.subcategoryHandle]);
      ...
      <div className="border-b border-black/[0.06] bg-surface-warm py-3">
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-2 text-sm text-ink-light">
            ...
            <ChevronRight className="w-3.5 h-3.5" />
            <Link href={`/categories/${params.handle}`} className="hover:text-amber-700 transition-colors">
              {categoryName}
            </Link>
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-ink font-medium">{subcategoryName}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-surface-warm">
        <div className="container mx-auto px-4 py-10">
          <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4 mb-8">
            <div>
              <p className="label mb-3">{categoryName}</p>
              <h1 className="font-heading text-display mb-2">{subcategoryName}</h1>
              {subcategoryDescription && (
                <p className="text-sm text-ink-light font-light max-w-2xl">{subcategoryDescription}</p>
              )}
